...
export const ServerHeader = ({
  ...
  const leaveServer = useMutation({
    mutationFn: async () => {
      ...
      const {
        error
      } = await supabase.from('server_members').delete().eq('server_id', server.id).eq('user_id', currentUser.id);
        ...
        <div className="px-4 py-2">
          <div className="flex items-center justify-between">
            ...
            y: 0
              ...
              {server.icon_url && <motion.img src={server.icon_url} alt={`${server.name} icon`} className="w-12 h-12 rounded-full border-2 border-background shadow-lg hover:scale-105 transition-transform" initial={{
              scale: 0.8
            }} animate={{
              scale: 1
            }} whileHover={{
            ...
            <div className="flex items-center gap-2 my-0 mx-0 py-0 px-[3px]">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {isOwner ? <DropdownMenuItem onClick={() => navigate(`/servers/${server.id}/settings`)}>
                      Server Settings
                    </DropdownMenuItem> : <>
                      ...
                      <DropdownMenuItem onClick={() => leaveServer.mutate()} className="text-red-600 focus:text-red-600">
                        Leave Server
                      </DropdownMenuItem>
                    </>}
                </DropdownMenuContent>
